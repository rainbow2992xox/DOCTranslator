...
class quene_task(models.Model):
    ...
    source_paragraph = fields.Many2one('doc.paragraph', 'Source paragraph', readonly=True)
    source_paragraph_idx = fields.Char(related='source_paragraph.paragraph_text_idx', string='Source paragraph text idx', readonly=True)

    source_text = fields.Text(related='source_paragraph.text', string='Source text', readonly=True)
    ...
    def start_task(self):
        align_tasks = self.env["doc.quene.task"].sudo().search([("status", "=", "1"), ("type", "=", "对齐")], order='create_date asc', limit=3)

        processing_align_tasks = self.env["doc.quene.task"].sudo().search([("status", "=", "2")], order='create_date asc')

        # 没有进行中的任务则执行
        if not processing_align_tasks.ids and align_tasks.ids:
            for rec in align_tasks:
                ...
                if align_res:
                    ...
                    for item in align_res:
                        self.env['doc.translation.paragraphs'].sudo().create({'target': item[getLabel(rec.target_lang, 'name')]['id'], 'source': item[getLabel(rec.source_lang, 'name')]['id'], 'score': item['score']})
        ...
        # 没有进行中的任务则执行
        processing_align_tasks = self.env["doc.quene.task"].sudo().search([("status", "=", "2")], order='create_date asc')
        translator_tasks = self.env["doc.quene.task"].sudo().search([("status", "=", "1"), ("type", "=", "翻译")], order='create_date asc', limit=32)
        if not processing_align_tasks.ids and translator_tasks.ids and not align_tasks.ids:
            ...
            msd_usr_ids = []
            for rec in translator_tasks:
                rec.status = '2'
                self.env.cr.commit()
                msd_usr_ids.append(rec.source_file.msd_usr_id)
            msd_usr_ids = list(set(msd_usr_ids))

            # 分组翻译
            for m in msd_usr_ids:
                texts = []
                ids = []
                for rec in translator_tasks:
                    if rec.source_file.msd_usr_id == m:
                ...
                msd_usr_id = m
                agent_res = agent_translate(texts, getLabel(rec.source_lang, 'name'), getLabel(rec.target_lang, 'name'), trans_technical_terms, msd_usr_id if msd_usr_id else None)

                idx = 0
                for rec_id in ids:
                    rec = self.env["doc.quene.task"].sudo().search([("id", "=", rec_id)])
                    rec.target_text = agent_res[idx]['result']
                    rec.score = agent_res[idx]['score']
                    rec.status = '3'
                    self.env.cr.commit()
                    idx += 1
